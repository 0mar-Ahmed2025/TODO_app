...
import 'package:to_do/features/auth/data/model/login_response_model.dart';
...
abstract class APIHelper {
  ...
  static Future<Either<String, UserModel>> getUserData() async {
    try {
      var userDataResponse = await _dio.get(
  ...
  static Future<Either<String, UserModel>> login({
    ...
    try {
      ...
      if (e is DioException) {
        String? myMessage;
        var errorResponse = e.response?.data as Map<String, dynamic>;
        if (errorResponse['message']?.contains('credentials') == true) {
          myMessage = 'Username Or Password Is Wrong\nPleas Try Again';
        }
        return Left(myMessage ?? errorResponse['message']);
      } else {
        return Left('An Error occurred.\nTry again later');
      }
    }
  ...
  static Future<Either<String, String>> register({
    ...
    try {
      ...
      var data = response.data as Map<String, dynamic>;
      return Right(data['message'] ?? 'Register Successfully');
    } catch (e) {
      if (e is DioException) {
        var data = e.response?.data as Map<String, dynamic>;
        return Left(data['message'] ?? 'Something Went Wrong');
  ...
  static Future<Either<String, String>> deleteTask({required int id}) async {
    try {
      ...
      var data = response.data as Map<String, dynamic>;
      return right(data['message'] ?? 'Task deleted successfully');
    } catch (e) {
      if (e is DioException) {
        var data = e.response?.data as Map<String, dynamic>;
        return left(data['message'] ?? 'Something went wrong');
